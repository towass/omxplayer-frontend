...
class Shutdown:
    def GET(self):
        subprocess.call('/root/shutdown -h now',shell=True)
        return '[{\"message\":\"OK\"}]'
...
def omx_play(file):
    ...
    subprocess.Popen('omxplayer --loop -o local '+os.path.join(MEDIA_RDIR,re.escape(file))+' <'+re.escape(OMXIN_FILE),shell=True)
    omx_send('z')
